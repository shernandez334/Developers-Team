...
public class UserDaoMySql implements UserDao {
    ...
    public User saveUser(User user) throws MySqlException, ExistingEmailException {
        ...
        if (user instanceof Player) {
            new NotificationsService(FactoryProvider.getInstance()).addSubscriber((Player) user);
        }
        return user;
    }
        ...
        try (QueryResult queryResult = createStatementAndExecuteQuery(sql)){
            ...
            if (!result.next()){
                ...
                user = new Player(result.getInt("user_id"), result.getString("name"),
                        password, email);
                new NotificationsService(FactoryProvider.getInstance()).loadNotificationsFromDatabase((Player) user);
            }else {
